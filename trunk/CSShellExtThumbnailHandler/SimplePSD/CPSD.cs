...
/* *************************************************
 * Photoshop parser in c++ originally devloped by ihaml http://www.codeproject.com/bitmap/MyPSD.asp
 * Enhancements Igor Tolmachev. http://www.codeproject.com/csharp/simplepsd.asp
 * Furthur enhancements with layers : Rajesh Lal connectrajesh@hotmail.com 
 * Date: 03/29/2007
...
namespace CSharpTricks.Photoshop
{
    ...
    public int Load(Stream stream)
    {
      ...
      return nErrorCode;
		...
		protected bool ReadHeader(Stream stream)
		{
			bool bSuccess = false;
		...
		protected bool ReadColourModeData(Stream stream)
		{
			bool bSuccess = false;
		...
		protected bool ReadImageResource(Stream stream)
		{
			bool bSuccess = false;
		...
		protected bool ReadLayerAndMaskInfoSection(Stream stream) // currently ignore it
		{
            bool bSuccess = false;
            int nTotalBytes = 0;
            //int nTotalSize =0;
            ...
            try
            {
                ...
                nSize = binReader.ReadBytes(4);
                //if(SwapBytes(nSize,4)) nTotalSize = BitConverter.ToInt32(nSize,0);
                nCount = binReader.ReadBytes(2);
                if (SwapBytes(nCount, 2)) nTotalCounts = BitConverter.ToInt16(nCount, 0);
                m_layerCount = nTotalCounts;

                if (nCount[0] > 50) m_layerCount = 1;

                if (stream.Position + nTotalBytes < stream.Length) stream.Position += nTotalBytes - 6;
                //				byte c = 0x00;
		...
		protected int ReadImageData(Stream stream)
		{
			int nErrorCode = 0;	// No Errors
